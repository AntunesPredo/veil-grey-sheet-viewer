...
const PlayerCard = React.memo(({ playerName }: { playerName: string }) => {
  ...
  return (
    <div className="border-2 border-[var(--theme-border)] bg-[var(--theme-background)] flex flex-col">
      <div className="bg-[var(--theme-danger)]/10 border-b-2 border-[var(--theme-danger)] p-2 flex justify-between items-center shrink-0">
        <span className="font-black text-[var(--theme-danger)] tracking-widest uppercase">
          UNIT: {playerName}
        </span>
        <span className="text-[10px] bg-[var(--theme-success)] text-black px-2 py-0.5 font-bold animate-pulse">
          ON-LINK
        </span>
      </div>
    ...
    <div className="flex flex-col h-full gap-4 overflow-y-auto custom-scrollbar pr-2">
      <div className="flex justify-between items-center border-b border-[var(--theme-danger)]/30 pb-2 shrink-0">
        <span className="text-xs font-bold text-[var(--theme-danger)] tracking-widest uppercase">
          VISÃO GERAL DO PELOTÃO
        </span>
        <span className="text-[10px] font-mono text-[var(--theme-text)]/50">
          CONEXÕES ATIVAS: {onlinePlayers.length}
        </span>
      </div>
      ...
      {onlinePlayers.length === 0 ? (
        <div className="flex-1 flex items-center justify-center border-2 border-dashed border-[var(--theme-danger)]/30 text-[var(--theme-danger)]/50 font-mono text-xs uppercase tracking-widest animate-pulse">
          NENHUMA UNIDADE CONECTADA AO CANAL OMEGA.
        </div>
      ) : (
